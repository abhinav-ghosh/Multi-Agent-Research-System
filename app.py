...
.panel-label {
    ...
    letter-spacing: 0.2em; text-transform: uppercase; margin-bottom: 1.2rem; padding-bottom: 0.7rem;
}
...
.result-panel-title {
    ...
    padding-bottom: 0.7rem; border-bottom: 1px solid rgba(255,140,50,0.15);
}
.result-content {
...
with col_input:
    ...
    for ex in ["LLM agents 2025", "CRISPR gene editing", "Fusion energy progress"]:
        ...
        border-radius:6px;padding:0.25rem 0.7rem;font-size:0.75rem;color:#a09890;">{ex}</span>
        """, unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
